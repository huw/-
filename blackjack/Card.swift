...
/**
This, and `Rank`, inherit from Int because Swift automatically assigns each a value in the order that they are allocated. With this, we can iterate over the enum to get a list of the possible cases.
*/
...
enum Rank: Int {
    ...
    /**
    This is one of my favourite functions. Because each case is assigned a `rawValue` based on its allocation order, we don't have to write cases saying that `Two = "2"`, or however it's done. Instead, we can simply get the raw value, and add 1 to it. Convert that to a String, and we're done!
...
class Card: SKSpriteNode {
    ...
    init(suit: Suit, rank: Rank) {
        ...
        super.init(texture: nil, color: NSColor.whiteColor(), size: CGSizeZero)
    }
